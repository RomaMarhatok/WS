...
class GenericRepository(Generic[SchemasTypes.SQLALCHEMY_MODEL_TYPE.value], ABC):
    ...
    async def save(
        self, dto: SchemasTypes.PYDANTIC_SCHEMA_TYPE.value
    ...
    async def delete(
        self, entity_or_uuididf: uuid.UUID | SchemasTypes.SQLALCHEMY_MODEL_TYPE.value
    ) -> bool:
    ...
    ) -> list[SchemasTypes.SQLALCHEMY_MODEL_TYPE.value]:
        async with self.session_factory() as session:
    ...
    ) -> SchemasTypes.SQLALCHEMY_MODEL_TYPE.value | list[Any] | Any:
        async with self.session_factory() as session:
            stmt = (
                Select(self.model)
    ...
    @classmethod
    @lru_cache(maxsize=1)
    def _get_entity_class(cls) -> Type[SchemasTypes.SQLALCHEMY_MODEL_TYPE.value]:
